...
class LMScorer(object):
    ...
    def load_model(cls, path, cpu=False):
        ...
        models, _ = utils.load_ensemble_for_inference(args.path.split(':'), task = task)
        models, _ = checkpoint_utils.load_model_ensemble(args.path.split(':'), args)
...
class GoodmanScoreCaculator(object):
    ...
    def distinctiveness(self):
        ...
        for i, w in enumerate(sent):
            ...
            kl_divs_p1_p2.append(self.kl_div(p1, p2))
            kl_divs_p2_p1.append(self.kl_div(p2, p1))
#             d = self.kl_div(p1, p2) + self.kl_div(p2, p1)
#             kl_divs.append(d)
        return np.sum(kl_divs_p1_p2), np.sum(kl_divs_p2_p1)
...
class GoodmanScorer(PunScorer):
    ...
    def analyze(self, pun_sent, pun_word, alter_word):
        ...
        distinctiveness_f1, distinctiveness_f2 = calculator.distinctiveness()

        res = {'ambiguity': ambiguity, 'distinctiveness_f1': distinctiveness_f1, 'distinctiveness_f2': distinctiveness_f2}
        res = {k: float(v) for k, v in res.items()}
        return res
